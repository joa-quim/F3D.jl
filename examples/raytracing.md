...
The interactive viewers in `examples/gmt_solids.jl` call
`_disable_raytracing_bindings(interactor)` right after
`F3D.f3d_interactor_init_bindings(interactor)` (4 sites: `view_image`, `view_grid`,
`view_points`). That removes the raytracing key binds.

To restore the live keys, in each of those sites replace
